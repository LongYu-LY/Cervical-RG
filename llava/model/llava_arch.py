...
class LlavaMetaModel:
    ...
    def initialize_vision_modules(self, model_args):
        ...
        self.config.vision_select_layer = model_args.mm_vision_select_layer
        self.config.vision_select_feature = model_args.mm_vision_select_feature
        ...
        print("使用了initialize_vision_modules")
        # vision tower
        ...
        if model_args.pretrain_vision_model is not None:
            def get_w(weights, keyword):
                return {keyword+'.' + k : v for k, v in weights.items()}
            vision_model_weights = torch.load(model_args.pretrain_vision_model, map_location='cpu')
            self.vision_tower.load_state_dict(get_w(vision_model_weights, 'vision_tower'), strict=True)
            self.vision_tower.requires_grad_(False) 
            print("pretrain vision model succeed loaded")
        ...
        if model_args.pretrain_mm_mlp_adapter is not None:
            ...
            print("pretrain vision adapter succeed loaded")
...
class LlavaMetaForCausalLM(ABC):
    ...
    # def encode_images(self, images, batch_size=150):
    #     num_images = len(images)
    #     image_features_list = []
    #     for i in range(0, num_images, batch_size):
    #         image_batch = images[i:i + batch_size]
    #         image_features = self.get_model().get_vision_tower()(image_batch)
    #         image_features_list.append(image_features)
    #         torch.cuda.empty_cache()
    #     # all_image_features = torch.cat(image_features_list, dim=0).requires_grad_(True)
    #     all_image_features = torch.cat(image_features_list, dim=0)
    #     all_image_features_final = self.get_model().mm_projector(all_image_features)
    #     return all_image_features_final
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        images, image_sizes=None, endoftext_token_id=2
    ):
        vision_tower = self.get_vision_tower()
        if vision_tower is None or images is None or input_ids.shape[1] == 1:
            return input_ids, position_ids, attention_mask, past_key_values, None, labels
        if type(images) is list or images.ndim == 5:
            images = [x.unsqueeze(0) if x.ndim == 4 else x for x in images]
