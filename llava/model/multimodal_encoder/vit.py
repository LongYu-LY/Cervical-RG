...
class ViT3DTower(nn.Module):
    def __init__(self, config):
        ...
        self.vision_tower_name = '/home/ly/Nas/ly/LLMs/hf_hub/Cervice_CLIP'
    ...
    def load_model(self, device_map=None):
        if self.is_loaded:
            ...
            self.vision_tower = AutoModel.from_pretrained(self.vision_tower_name,trust_remote_code=True).vision_encoder
            self.vision_tower.requires_grad_(False)        
            print("vision_tower是",self.vision_tower.state_dict())
    ...
    def forward(self, images):
        ...
        if type(images) is list:
            image_features = []
            for image in images:
                last_feature, hidden_states = self.vision_tower(image.to(device=self.device, dtype=self.dtype).unsqueeze(0))
                if self.select_layer == -1:
                    image_feature = last_feature
                elif self.select_layer < -1:
                    image_feature = self._select_features(last_feature, hidden_states)
                else:
                    raise ValueError(f'Unexpected select layer: {self.select_layer}')

                # image_feature = self.feature_select(image_features).to(image.dtype)
                torch.cuda.empty_cache()
                image_features.append(image_feature)
        else:
            last_feature, hidden_states = self.vision_tower(images.to(device=self.device, dtype=self.dtype))
         # 选择特征
            image_features = self._select_features(last_feature, hidden_states)

        return image_features

    def _select_features(self, last_feature, hidden_states):
        if self.select_layer == -1:
