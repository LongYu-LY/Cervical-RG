...
class ViT3DTower(nn.Module):
    def __init__(self, config):
        ...
        self.vision_tower_name = '/home/ly/LLMs/hf_hub/models--GoodBaiBai88--M3D-CLIP'
        self.image_processor = CLIPImageProcessor.from_pretrained(self.vision_tower_name)
        self.select_feature = config.mm_vision_select_feature
        print(config.image_channel)
    ...
    def forward(self, images):
        ...
        print("imagestype:",type(images),"imagesshape",images.shape)
        
        if type(images) is list:
            image_features = []
            for image in images:
                last_feature, hidden_states = self.vision_tower(image.to(device=self.device, dtype=self.dtype).unsqueeze(0), output_hidden_states=True)
            ...
            last_feature, hidden_states = self.vision_tower(images.to(device=self.device, dtype=self.dtype), output_hidden_states=True)
         # 选择特征
        ...
        # print("Output Example:", image_features[0])  # 查看第一个样本的输出
